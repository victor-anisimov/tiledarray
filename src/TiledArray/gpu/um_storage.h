...
#ifndef TILEDARRAY_SYCL_UM_VECTOR_H__INCLUDED
#define TILEDARRAY_SYCL_UM_VECTOR_H__INCLUDED
...
void to_execution_space(Storage& vec, sycl::queue* stream = 0) {
  switch (Space) {
    case ExecutionSpace::CPU: {
      ...
      if (cudaEnv::instance()->concurrent_managed_access()) {
        ...
        */
        CudaSafeCall(
            (stream->prefetch(data(vec), size(vec) * sizeof(value_type)), 0));
      }
      break;
    }
    case ExecutionSpace::CUDA: {
      ...
      int device = -1;
      if (cudaEnv::instance()->concurrent_managed_access()) {
        CudaSafeCall(device = dpct::dev_mgr::instance().current_device_id());
        /*
...
#endif  // TILEDARRAY_SYCL_UM_VECTOR_H__INCLUDED
