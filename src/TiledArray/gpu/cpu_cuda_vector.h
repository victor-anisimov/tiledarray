...
#ifndef TILEDARRAY_SYCL_CPU_SYCL_VECTOR_H__INCLUDED
#define TILEDARRAY_SYCL_CPU_SYCL_VECTOR_H__INCLUDED
...
class cpu_cuda_vector {
 public:
  ...
  typedef typename std::vector<T, HostAlloc>::size_type size_type;
  typedef typename std::vector<T, HostAlloc>::difference_type difference_type;
  typedef typename std::vector<T, HostAlloc>::iterator iterator;
  typedef typename std::vector<T, HostAlloc>::const_iterator const_iterator;
  ...
  const T* device_data() const {
    assert(on_device());
    return dpct::get_raw_pointer(device_vec_.data());
  }
  T* device_data() {
    assert(on_device());
    state_ = state::device;
 ...
 private:
  mutable std::vector<T, HostAlloc> host_vec_;
  mutable dpct::device_vector<T, DeviceAlloc> device_vec_;
...
#endif  // TILEDARRAY_SYCL_CPU_SYCL_VECTOR_H__INCLUDED
